...
def test_mixed_types_tuple():
    obj = (
        (
            ...
            (b'a', 10 ** 20), tuple(range(-100000, 100000)),
            b'b' * 31, b'b' * 62, b'b' * 64, 2 ** 30,
            2 ** 33, 2 ** 62, 2 ** 64, 2 ** 30, 2 ** 33, 2 ** 62, 2 ** 64,
            False, False, True,
            -1, 2, 0,
            None,
        ),
...
def test_tuple_of_bytes():
    obj = (
        b'', b'a' * 10, b'a' * 100, b'a' * 1000, b'a' * 10000,
        b'a' * 100000, b'a' * 1000000, b'a' * 10000000
    ) + tuple([b'a' * n for n in range(1000)]) + (b'b',)
    assert loads(dumps(obj)) == obj


def test_tuple_of_dicts():
    obj = tuple(
        [dict(zip(range(n), range(n))) for n in range(100)]
    ) + tuple(
        [dict(zip(range(n), range(-n, 0))) for n in range(100)]
    ) + (b'b',)
    assert loads(dumps(obj)) == obj
